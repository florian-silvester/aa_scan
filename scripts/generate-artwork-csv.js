...
async function generateArtworkCSV() {
  ...
  const rows = ['Item ID,Work Title,Slug,Description'];
  let matched = 0;
  let unmatched = 0;
  ...
  for (const item of allItems) {
    ...
    if (sanityItem) {
      const title = sanityItem.workTitle?.de || sanityItem.workTitle?.en || item.fieldData['work-title'] || '';
      const desc = extract(sanityItem.description?.de) || extract(sanityItem.description?.en) || '';
      
      const fields = [itemId, title, slug, desc];
      const csvLine = fields.map(f => `"${String(f).replace(/"/g, '""')}"`).join(',');
      rows.push(csvLine);
      matched++;
      if (matched <= 10) console.log(`✓ ${slug}: ${title}`);
    } else {
      const title = item.fieldData['work-title'] || '';
      const fields = [itemId, title, slug, ''];
      const csvLine = fields.map(f => `"${String(f).replace(/"/g, '""')}"`).join(',');
      rows.push(csvLine);
      unmatched++;
  ...
  console.log(`Note: This is 896 items, may take a few minutes to import`);
}
