...
async function generateMaterialCSV() {
  ...
  console.log(`Found ${enItems.items.length} EN materials`);
  console.log(`Found ${sanityMaterials.length} Sanity materials`);
  ...
  for (const item of enItems.items) {
    ...
    // Use DE name from Sanity, or fallback to EN name
    const name = sanityItem?.name?.de || sanityItem?.name?.en || item.fieldData.name;
  ...
  const csv = rows.join('\n');
  const outPath = path.join(__dirname, '..', 'material-de.csv');
  fs.writeFileSync(outPath, csv, 'utf8');

  console.log(`\n✅ Generated: ${outPath}`);
  console.log(`${rows.length - 1} materials ready for DE import`);
  console.log(`\nNext steps:`);
  console.log(`1. Import material-de.csv to Material collection`);
  console.log(`2. Select DE-DE locale`);
  console.log(`3. Choose "Link and update matching items"`);
}

generateMaterialCSV().catch(console.error);
