...
async function generateMaterialDECSV() {
  ...
  const csv = rows.join('\n');
  const outPath = path.join(__dirname, '..', 'reports', 'material-de.csv');
  fs.writeFileSync(outPath, csv, 'utf8');

  console.log(`\n✅ Generated: ${outPath}`);
  console.log(`${rows.length - 1} materials with GERMAN names`);
}

generateMaterialDECSV().catch(console.error);
