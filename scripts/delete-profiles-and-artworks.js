...
async function deleteCreatorsAndArtworks() {
  try {
    ...
    console.log('Creators to be deleted:');
    creatorsToDelete.forEach((creator, index) => {
      console.log(`  ${index + 1}. ${creator.name} (${creator.artworks.length} artworks)`);
    });

    console.log('\n⚠️  WARNING: This will permanently delete these creators and all their artworks!');
